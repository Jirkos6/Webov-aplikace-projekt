<div>
    ...
    <x-form-section submit="createApiToken">
        <x-slot name="title">
            {{ __('Vytvořte API Token') }}
        </x-slot>

        <x-slot name="description">
            {{ __('API tokeny pomáhají autentizaci pomocí aplikací třetích stran jako Microsoft Authenticator') }}
        </x-slot>

        <x-slot name="form">
            <!-- Token Name -->
            <div class="col-span-6 sm:col-span-4">
                <x-label for="name" value="{{ __('Jméno tokenu') }}" />
            ...
            @if (Laravel\Jetstream\Jetstream::hasPermissions())
                <div class="col-span-6">
                    <x-label for="permissions" value="{{ __('Práva') }}" />
    ...
    @if ($this->user->tokens->isNotEmpty())
        ...
        <div class="mt-10 sm:mt-0">
            <x-action-section>
                <x-slot name="title">
                    {{ __('Spravovat API Tokeny') }}
                </x-slot>

                <x-slot name="description">
                    {{ __('Můžete vymazat svoje API Tokeny pokud je už nepotřebujete') }}
                </x-slot>
    ...
    <x-dialog-modal wire:model.live="displayingToken">
        ...
        <x-slot name="content">
            <div>
                {{ __('Prosím zkopírujte svůj API Token. Kvůli bezpečnostím důvodům ho znovu neukážeme.') }}
            </div>
    ...
    <x-confirmation-modal wire:model.live="confirmingApiTokenDeletion">
        ...
        <x-slot name="content">
            {{ __('Opravdu chceš smazat tento API Token?') }}
        </x-slot>
